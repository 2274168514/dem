...
function initializeApp() {
  ...
  try {
    ...
    // 初始化通知管理器（单例模式）- 仅当通知按钮存在时
    try {
      const notificationBtn = document.getElementById('notification-btn');
      if (notificationBtn && !window.notificationManager) {
        const notificationManager = new NotificationManager();
        window.notificationManager = notificationManager;
        console.log('✅ 通知管理器初始化成功');
      } else if (!notificationBtn) {
        console.log('ℹ️ 通知按钮不存在，跳过通知管理器初始化');
      } else {
        console.log('ℹ️ 通知管理器已存在，跳过重复初始化');
      }
    } catch (error) {
      console.warn('⚠️ 通知管理器初始化失败:', error);
    }
